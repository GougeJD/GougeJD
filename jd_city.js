...
[task_local]
#城城领现金
0 0 * * * jd_city.js, tag=城城领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

=================================Loon===================================
[Script]
cron "0 0 * * *" script-path=jd_city.js,tag=城城领现金

===================================Surge================================
城城领现金 = type=cron,cronexp="0 0 * * *",wake-system=1,timeout=3600,script-path=jd_city.js

====================================小火箭=============================
城城领现金 = type=cron,script-path=jd_city.js, cronexpr="0 0 * * *", timeout=3600, enable=true
 */
const $ = new Env('城城领现金');
...
let inviteCodes = [
'RtGKzbrxFA2gLNaYEYI1gGvZKlFkVHJ47cZo3h5mpY4tqdOc5Q@RtGKzer3RgigeYeaRNw2hxuLjAh28rZYxtmPUocnQWv7-iP0Qw@RtGKzOumRlyhKIeTFdZi1oLNrom5nVkoTeEzDVkXIOYN6D5GkQ'
]
!(async () => {
  if (!cookiesArr[0]) {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://api.jd.xh-yww.cn/code/city`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
